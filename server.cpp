...
void *BoardcastThread(void* ptr) {
	...
	while (1) {
		...
		if (-1 != g_thread_info[thread_num].thr_socketfd) {
			...
			for (int i = 0; i < 10; i++) {
				if (-1 != g_user_socket_list[i] && fd != g_user_socket_list[i]) {
    		...
    		if (events[i].data.fd == listenfd) {
    			...
    			for (int n = 0; n < 10; n++) {
    				if (-1 == g_user_socket_list[n]) {
    					...
    					for (int m=0; m <10; m++) {
    						if(-1 != g_user_socket_list[m] && accepted_fd != g_user_socket_list[m]) {
    							write(g_user_socket_list[m],online_message,64);
    						}
    					}
    					break;
    				}
